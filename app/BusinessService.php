...
class BusinessService extends SeoModel
{
    ...
    public static function simpan( $request )
    {
        ...
        if( $request->hasFile('image') ){
            if( $request->file('image')->isValid() )
            {
                ...
                $request->file('image')->move(public_path().'/files/services', $imgUrl);
                $thumb = Image::make(public_path().'/files/services/'.$imgUrl);
                $thumb->resize(120, 120);
                $thumb->save(public_path().'/files/services/thumbs/'.$imgUrl);
    ...
    {
        return "SRV-".Carbon::now()->format('dmyhis').str_random(5);
    }
